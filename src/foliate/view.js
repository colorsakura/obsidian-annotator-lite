// Copied from https://github.com/johnfactotum/foliate-js/blob/main/view.js
// This is the GitHub version which includes PDF support via isPDF() check.
// All dynamic imports from './xxx.js' are redirected to 'foliate-js/xxx.js'
// via an esbuild onResolve plugin, except './pdf.js' → pdf-book.ts
...
export const makeBook = async file => {
    ...
    if (file.isDirectory) {
        ...
        // Redirected to our adapter by esbuild onResolve plugin
        const { makePDF } = await import('./pdf-book.ts')
